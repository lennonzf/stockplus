<?php
	...
	//Sold Record
	$query_sold = "SELECT year(Date) AS year, month(Date) AS month, Sum(SoldQTY*Cost) AS product_cost, Sum(SoldQTY*Price) AS product_sale, Sum(soldQTY*(Price-Cost)) AS product_profit FROM SoldRecord NATURAL JOIN Product WHERE ShopID = '".$shopid."' GROUP BY month ORDER BY year, month DESC ";
	...
	if(mysqli_num_rows($result_sold) == 0){
		...
		while($row = mysqli_fetch_array($result_sold)){
			...
			//find refund record
			$query_refund = "SELECT Barcode, ProductName, Sum(Cost) AS refundcost, Sum(Price) AS refundsale FROM RefundRecord NATURAL JOIN Product WHERE year(Date) = '".$year."' AND month(Date) = '".$month."' AND ShopID = '".$shopid."' ";
			...
			//extra
			$query_extra = "SELECT Sum(cost) AS cost FROM ExtraExpense WHERE year(Date) = '".$year."' AND month(Date) = '".$month."' AND ShopID = '".$shopid."' ";
			...
			//dynamic
			$query_dynamic = "SELECT Sum(Labour) AS labour, Sum(Rent) AS rent, Sum(Discount) AS discount FROM DynamicExpense WHERE year(Date) = '".$year."' AND month(Date) = '".$month."' AND ShopID = '".$shopid."' "; 
