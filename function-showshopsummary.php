<?php
	...
	//Sold Record
	$query_sold = "SELECT Sum(SoldQTY*Cost) AS product_cost, Sum(SoldQTY*Price) AS product_sale, Sum(soldQTY*(Price-Cost)) AS product_profit, Date FROM SoldRecord NATURAL JOIN Product WHERE ShopID = '".$shopid."' AND Date <= '".$date_to."' AND Date >= '".$date_from."' ";
	...
	if(mysqli_num_rows($result_sold) == 0){
		...
		while($row = mysqli_fetch_array($result_sold)){
			...
			//find refund record
			$query_refund = "SELECT Barcode, ProductName, Cost, Price FROM RefundRecord NATURAL JOIN Product WHERE ShopID = '".$shopid."' AND Date = '".$date."' ";
	...
	//Dynamic
	$query_dynamic = "SELECT Sum(Labour) AS totallabour, Sum(Rent) AS totalrent, Sum(discount) AS totaldiscount FROM DynamicExpense WHERE ShopID = '".$shopid."' AND Date <= '".$date_to."' AND Date >= '".$date_from."' ";
	...
	//Extra
	$query_extra = "SELECT Sum(Cost) AS totalextra FROM ExtraExpense WHERE ShopID = '".$shopid."' AND Date <= '".$date_to."' AND Date >= '".$date_from."' ";
